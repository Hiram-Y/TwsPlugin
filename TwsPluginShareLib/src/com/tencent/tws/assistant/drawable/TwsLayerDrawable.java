 ...
 * <p>
 * It can be defined in an XML file with the <code>&lt;layer-list></code>
 * element. Each Drawable in the layer is defined in a nested
 * <code>&lt;item></code>.
 * <p>
...
public class TwsLayerDrawable extends TwsDrawable implements Drawable.Callback {
	/**
	 ...
	 *            must be non-null
	 */
	public TwsLayerDrawable(@NonNull Drawable[] layers) {
		this(layers, null);
	}
	 ...
	 */
	TwsLayerDrawable(@NonNull Drawable[] layers, @Nullable LayerState state) {
		this(state, null);
	...
	TwsLayerDrawable(@Nullable LayerState state, @Nullable Resources res) {
	...
	public void inflate(Resources r, XmlPullParser parser, AttributeSet attrs) throws XmlPullParserException, IOException {
		...
		inflateLayers(r, parser, attrs, null);
	...
	private void updateStateFromTypedArray(TypedArray a) {
		...
		for (int i = 0; i < N; i++) {
			...
			if (attr == R.styleable.LayerDrawable_opacity)
				state.mOpacityOverride = a.getInt(attr, state.mOpacityOverride);
			else if (attr == R.styleable.LayerDrawable_opacity)
	...
	private void inflateLayers(Resources r, XmlPullParser parser, AttributeSet attrs, Theme theme) throws XmlPullParserException, IOException {
		...
		while ((type = parser.next()) != XmlPullParser.END_DOCUMENT && ((depth = parser.getDepth()) >= innerDepth || type != XmlPullParser.END_TAG)) {
			...
			final TypedArray a = r.obtainAttributes(attrs, R.styleable.LayerDrawableItem);
	 ...
	 * <p>
	 * <strong>Note:</strong> Setting an explicit layer size changes the default
	 * layer gravity behavior. See {@link #setLayerGravity(int, int)} for more
	 * information.
	 * 
	 * @param index
	 ...
	 * behavior depends on whether an explicit width or height has been set (see
	 * {@link #setLayerSize(int, int, int)}), If a dimension is not set, gravity
	 * in that direction defaults to {@link Gravity#FILL_HORIZONTAL} or
	...
	/**
	 * Specifies the relative insets in pixels for the drawable at the specified
	 * index.
	 * 
	 * @param index
	 *            the index of the layer to adjust
	...
	/*
	 * @Override public void setHotspot(float x, float y) { final
	 * ChildDrawable[] array = mLayerState.mChildren; final int N =
	 * mLayerState.mNum; for (int i = 0; i < N; i++) { final Drawable dr =
	 * array[i].mDrawable; if (dr != null) { dr.setHotspot(x, y); } } }
	 * 
	 * @Override public void setHotspotBounds(int left, int top, int right, int
	 * bottom) { final ChildDrawable[] array = mLayerState.mChildren; final int
	 * N = mLayerState.mNum; for (int i = 0; i < N; i++) { final Drawable dr =
	 * array[i].mDrawable; if (dr != null) { dr.setHotspotBounds(left, top,
	 * right, bottom); } }
	 * 
	 * if (mHotspotBounds == null) { mHotspotBounds = new Rect(left, top, right,
	 * bottom); } else { mHotspotBounds.set(left, top, right, bottom); } }
	 * 
	 * @Override public void getHotspotBounds(Rect outRect) { if (mHotspotBounds
	 * != null) { outRect.set(mHotspotBounds); } else {
	...
	/*
	 * @Override public int getAlpha() { final Drawable dr =
	 * getFirstNonNullDrawable(); if (dr != null) { return dr.getAlpha(); } else
	...
	/*
	 * @Override public void setTintList(ColorStateList tint) { final
	 * ChildDrawable[] array = mLayerState.mChildren; final int N =
	 * mLayerState.mNum; for (int i = 0; i < N; i++) { final Drawable dr =
	 * array[i].mDrawable; if (dr != null) { dr.setTintList(tint); } } }
	 * 
	 * @Override public void setTintMode(Mode tintMode) { final ChildDrawable[]
	 * array = mLayerState.mChildren; final int N = mLayerState.mNum; for (int i
	 * = 0; i < N; i++) { final Drawable dr = array[i].mDrawable; if (dr !=
	 * null) { dr.setTintMode(tintMode); } } }
	 */
	 ...
	 */
	/*
	 * public void clearMutated() { super.clearMutated();
	 * 
	 * final ChildDrawable[] array = mLayerState.mChildren; final int N =
	 * mLayerState.mNum; for (int i = 0; i < N; i++) { final Drawable dr =
	 * array[i].mDrawable; if (dr != null) { dr.clearMutated(); } } mMutated =
	 * false; }
	 * 
	 * @Override public boolean onLayoutDirectionChanged(@View.ResolvedLayoutDir
	 * int layoutDirection) { boolean changed = false;
	 * 
	 * final ChildDrawable[] array = mLayerState.mChildren; final int N =
	 * mLayerState.mNum; for (int i = 0; i < N; i++) { final Drawable dr =
	 * array[i].mDrawable; if (dr != null) { changed |=
	 * dr.setLayoutDirection(layoutDirection); } }
	 * 
	 * updateLayerBounds(getBounds()); return changed; }
	 */
	...
	static class ChildDrawable {
		...
		/*
		 * public boolean canApplyTheme() { return mThemeAttrs != null ||
	...
	static class LayerState extends ConstantState {
		...
		LayerState(LayerState orig, TwsLayerDrawable owner, Resources res) {
			if (orig != null) {
		...
		public Drawable newDrawable() {
			return new TwsLayerDrawable(this, null);
		}

		@Override
		public Drawable newDrawable(Resources res) {
			return new TwsLayerDrawable(this, res);
		}
		...
		/*
		 * @Override public int addAtlasableBitmaps(Collection<Bitmap>
		 * atlasList) { final ChildDrawable[] array = mChildren; final int N =
		 * mNum; int pixelCount = 0; for (int i = 0; i < N; i++) { final
		 * Drawable dr = array[i].mDrawable; if (dr != null) { final
		 * ConstantState state = dr.getConstantState(); if (state != null) {
		 * pixelCount += state.addAtlasableBitmaps(atlasList); } } } return
		 * pixelCount; }
		 */
	}
