...
public class DepthLayout extends RelativeLayout {
  ...
  public static final int[] STYLEABLE =
      new int[] { R.attr.is_circle, R.attr.edge_color, R.attr.depth, R.attr.custom_elevation };
  ...
  boolean hasMatrixChanged(float[] newSrc) {
    for (int i = 0; i < 8; i++) {
      if (newSrc[i] != prevSrc[i]) return true;
    }
    return false;
  }
  ...
  class CustomShadow {
    ...
    public boolean calculateBounds(DepthLayout target) {
      ...
      float[] dst = new float[] {
          -padding, -padding, target.getWidth() + padding, -padding, -padding,
          target.getHeight() + padding, target.getWidth() + padding, target.getHeight() + padding
      };
      Matrix matrix = getMatrix();
      matrix.mapPoints(src, dst);
    ...
    public void drawShadow(Canvas canvas, DepthLayout dl, Drawable shadow) {
      ...
      float[] src =
          new float[] { 0, 0, dl.getWidth(), 0, dl.getWidth(), dl.getHeight(), 0, dl.getHeight() };
      float[] dst = new float[] {
          topLeftBack.x, topLeftBack.y, topRightBack.x, topRightBack.y, bottomRightBack.x,
          bottomRightBack.y, bottomLeftBack.x, bottomLeftBack.y
      };
