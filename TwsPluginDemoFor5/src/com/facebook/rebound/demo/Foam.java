...
public class Foam extends PathBitmapMesh {
	...
	public Foam(int horizontalSlices, int verticalSlices, Bitmap bitmap, float minHeight, float maxHeight,
			int animDuration) {
	...
	public void matchVertsToPath(Path path, float extraOffset) {
		...
		for (int i = 0; i < staticVerts.length / 2; i++) {
			...
			if (yIndexValue == 0) {
				...
				float desiredYCoord = Math.max(coords2[1],
						coords2[1] + easedFoamCoords[Math.min(easedFoamCoords.length - 1, index)]);
