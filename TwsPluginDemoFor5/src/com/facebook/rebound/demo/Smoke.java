...
public class Smoke extends Renderable {
  ...
  public Smoke(Bitmap bitmap, float x, float y, float height, float width, int numberOfTurns,
      float density) {
    super(bitmap, x, y);
    this.height = height;
    this.width = width;
    ...
    pathPointOffsetAnim = ValueAnimator.ofFloat(0,
        ((bitmap.getHeight() / (float) numberOfTurns) * 2f) / bitmap.getHeight()).setDuration(1500);
  ...
  private void createPath() {
    ...
    for (int i = 0; i < numberOfTurns; i++) {
      if (goLeft) {
        smokePath.cubicTo(x, y - step * i, x + width, y - step * i - step / 2, x,
            y - step * i - step);
      } else {
        smokePath.cubicTo(x, y - step * i, x - width, y - step * i - step / 2, x,
            y - step * i - step);
  ...
  private void matchVertsToPath(float wind) {
    ...
    for (int i = 0; i < staticVerts.length / 2; i++) {
      ...
      float percentOffsetY2 =
          (0.000001f + yIndexValue) / (bitmap.getHeight() + ((bitmap.getHeight() / numberOfTurns)
              * 4f));
      ...
      if (xIndexValue == 0) {
        ...
        desiredXCoord += (wind / 3f) * density + ((wind * WIND_SENSITIVITY) * (1f
            - smokeExponentionWindStuff.getInterpolation(percentOffsetY)));
