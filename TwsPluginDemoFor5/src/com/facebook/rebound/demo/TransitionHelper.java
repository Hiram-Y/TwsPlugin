package com.facebook.rebound.demo;
...
public class TransitionHelper {
	...
	static ObjectAnimator introAnimate(final DepthLayout target, final float moveY, final float customElevation,
			long delay, int subtractDelay) {
		...
		ObjectAnimator translationY2 = ObjectAnimator.ofFloat(target, View.TRANSLATION_Y,
		...
		ObjectAnimator translationX2 = ObjectAnimator.ofFloat(target, View.TRANSLATION_X,
				-target.getResources().getDisplayMetrics().widthPixels, 0).setDuration(800);
		...
		ObjectAnimator rotationX = ObjectAnimator.ofFloat(target, View.ROTATION_X, TARGET_ROTATION_X, 0).setDuration(
				1000);
		...
		ObjectAnimator elevation = ObjectAnimator.ofFloat(target, "CustomShadowElevation",
				customElevation * target.getResources().getDisplayMetrics().density, target.getCustomShadowElevation())
				.setDuration(1000);
		...
		ObjectAnimator scaleX = ObjectAnimator.ofFloat(target, View.SCALE_X, TARGET_SCALE, target.getScaleX())
				.setDuration(1000);
		...
		ObjectAnimator scaleY = ObjectAnimator.ofFloat(target, View.SCALE_Y, TARGET_SCALE, target.getScaleY())
				.setDuration(1000);
	...
	private static void hideStatusBar(View root) {
		...
		// root.getContext()).getWindow().getDecorView();
		// int uiOptions = View.SYSTEM_UI_FLAG_FULLSCREEN;
		// decorView.setSystemUiVisibility(uiOptions);
	}

	static ValueAnimator exitAnimate(final DepthLayout target, final float moveY, final float customElevation,
			long delay, int subtractDelay, boolean continueOffscreen) {

		target.setPivotY(getDistanceToCenter(target));
		target.setPivotX(getDistanceToCenterX(target));
		target.setCameraDistance(10000 * target.getResources().getDisplayMetrics().density);

		ObjectAnimator rotationX = ObjectAnimator.ofFloat(target, View.ROTATION_X, TARGET_ROTATION_X).setDuration(
				DURATION);
		...
		ObjectAnimator elevation = ObjectAnimator.ofFloat(target, "CustomShadowElevation",
				target.getCustomShadowElevation(), customElevation * target.getResources().getDisplayMetrics().density)
		...
		ObjectAnimator translationY = ObjectAnimator.ofFloat(target, View.TRANSLATION_Y,
				-moveY * target.getResources().getDisplayMetrics().density).setDuration(subtractDelay);
	...
	private static void continueOutToRight(DepthLayout target, float moveY, int subtractDelay) {
		ObjectAnimator translationY2 = ObjectAnimator.ofFloat(target, View.TRANSLATION_Y,
				-moveY * target.getResources().getDisplayMetrics().density,
				-target.getResources().getDisplayMetrics().heightPixels).setDuration(900);
		translationY2.setInterpolator(new ExpoIn());
		translationY2.setStartDelay(0 + subtractDelay);

		translationY2.start();

		ObjectAnimator translationX2 = ObjectAnimator.ofFloat(target, View.TRANSLATION_X, target.getTranslationX(),
				target.getResources().getDisplayMetrics().widthPixels).setDuration(900);
	...
	public static void animateToMenuState(View root, AnimatorListenerAdapter onMenuAnimFinished) {
		...
		exitAnimate((DepthLayout) root.findViewById(R.id.dl3), MOVE_Y_STEP * 2, 20f, 75, 250, false).addListener(
				onMenuAnimFinished);

		ObjectAnimator translationY = ObjectAnimator.ofFloat(root, View.TRANSLATION_Y,
				-90f * root.getResources().getDisplayMetrics().density).setDuration(DURATION);
		translationY.setInterpolator(VALUEinterpolator);
		translationY.start();
	}
	...
	private static void makeAppFullscreen(View target) {
		// ((Activity)
		// target.getContext()).getWindow().setStatusBarColor(Color.TRANSPARENT);
		// ((Activity)
		// target.getContext()).getWindow().getDecorView().setSystemUiVisibility(
	...
	static ObjectAnimator revertFromMenu(final DepthLayout target, final float customElevation, int subtractDelay,
		...
		ObjectAnimator rotationX = ObjectAnimator.ofFloat(target, View.ROTATION_X, target.getRotationX(), 0)
				.setDuration(1000);
		rotationX.setInterpolator(new QuintInOut());
		...
		ObjectAnimator elevation = ObjectAnimator.ofFloat(target, "CustomShadowElevation",
				target.getCustomShadowElevation(), targetElevation * target.getResources().getDisplayMetrics().density)
				.setDuration(1000);
		elevation.setInterpolator(new QuintInOut());
		...
		ObjectAnimator rotation = ObjectAnimator.ofFloat(target, View.ROTATION, target.getRotation(), 0).setDuration(
				1100);
