        ...
        try {
            ...
            if (simpleMode) {
                ...
                if (certs == null) {
					...
					TwsLog.e(TAG, "INSTALL_PARSE_FAILED_NO_CERTIFICATES");
                    return null;
                }
                if (DEBUG) {
					TwsLog.d(TAG, "File " + sourcePath + ": entry=" + jarEntry + " certs=" + (certs != null ? certs.length : 0));
                    if (certs != null) {
                        final int N = certs.length;
                        for (int i=0; i<N; i++) {
							TwsLog.d(TAG, "  Public key: " + certs[i].getPublicKey().getEncoded() + " " + certs[i].getPublicKey());
                        }
                    }
                }
                ...
                while (entries.hasMoreElements()) {
                    ...
                    Certificate[] localCerts = loadCertificates(jarFile, je,
                            readBuffer);
                    if (DEBUG) {
						TwsLog.d(TAG, "File " + sourcePath + " entry " + je.getName() + ": certs=" + certs + " (" + (certs != null ? certs.length : 0) + ")");
                    }
                    if (localCerts == null) {
						TwsLog.d(TAG, "Package " + " has no certificates at entry " + je.getName() + "; ignoring!");
                        jarFile.close();
                        ...
                        for (int i=0; i<certs.length; i++) {
                            ...
                            if (!found || certs.length != localCerts.length) {
								TwsLog.e(TAG, "Package " + " has mismatched certificates at entry " + je.getName() + "; ignoring!");
                                jarFile.close();

								TwsLog.e(TAG, "INSTALL_PARSE_FAILED_INCONSISTENT_CERTIFICATES");
                                return null;
                            }
                        }
                    }
			...
			TwsLog.e(TAG, "INSTALL_PARSE_FAILED_CERTIFICATE_ENCODING");
            return null;
        } catch (IOException e) {
			TwsLog.e(TAG, "Exception reading " + sourcePath, e);
			TwsLog.e(TAG, "INSTALL_PARSE_FAILED_CERTIFICATE_ENCODING");
			...
			TwsLog.e(TAG, "Exception reading " + sourcePath, e);
			TwsLog.e(TAG, "INSTALL_PARSE_FAILED_UNEXPECTED_EXCEPTION");
            return null;
        }
