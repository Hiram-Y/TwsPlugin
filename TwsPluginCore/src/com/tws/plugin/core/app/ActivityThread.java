...
public class ActivityThread {
	...
	private static final String TAG = "rick_Print:Core::ActivityThread";
	...
	public synchronized static Object currentActivityThread() {
		if (sCurrentActivityThread == null) {
			// 从ThreadLocal中取出来的
			TwsLog.d(TAG, "从宿主程序中取出ActivityThread对象备用");

			sCurrentActivityThread = RefInvoker.invokeStaticMethod(android_app_ActivityThread,
					android_app_ActivityThread_currentActivityThread, (Class[]) null, (Object[]) null);
			...
			if (sCurrentActivityThread == null) {
				...
				if (impl != null) {
					sCurrentActivityThread = RefInvoker.getFieldObject(impl, android_app_ContextImpl,
							android_app_ContextImpl_mMainThread);
				}
			}
		}
	...
	public static Object getResCompatibilityInfo() {
		...
		Object compatInfo = RefInvoker.getFieldObject(mBoundApplication, android_app_ActivityThread_AppBindData,
				"compatInfo");
		return compatInfo;
	}
	...
	public static Object getBoundApplicationData() {
		Object mBoundApplication = RefInvoker.getFieldObject(currentActivityThread(), android_app_ActivityThread,
				"mBoundApplication");
		return mBoundApplication;
	}

	public static void installContentProviders(Context context, List<ProviderInfo> providers) {
		RefInvoker.invokeMethod(currentActivityThread(), clazz(), android_app_ActivityThread_installContentProviders,
				new Class[] { Context.class, List.class }, new Object[] { context, providers });
	}

	public static void wrapHandler() {
		Handler handler = (Handler) RefInvoker.invokeMethod(currentActivityThread(), clazz(),
				android_app_ActivityThread_getHandler, (Class[]) null, (Object[]) null);
		RefInvoker.setFieldObject(handler, Handler.class.getName(), android_os_Handler_mCallback, new PluginAppTrace(
				handler));
	}
	...
	public static void wrapInstrumentation() {
		...
		if (!(originalInstrumentation instanceof PluginInstrumentionWrapper)) {
			RefInvoker.setFieldObject(currentActivityThread(), clazz(), android_app_ActivityThread_mInstrumentation,
	...
	public static void installPackageInfo(Context hostContext, String pluginId, PluginDescriptor pluginDescriptor,
		...
		Object applicationLoaders = RefInvoker.invokeStaticMethod("android.app.ApplicationLoaders", "getDefault",
				(Class[]) null, (Object[]) null);
		Map mLoaders = (Map) RefInvoker
				.getFieldObject(applicationLoaders, "android.app.ApplicationLoaders", "mLoaders");
		if (mLoaders == null) {
			// what!!
			return;
		}
		...
		try {
			...
			// 会触发替换
			Object pluginLoadedApk = RefInvoker.invokeMethod(currentActivityThread(), android_app_ActivityThread,
			...
			if (pluginLoadedApk != null) {
				...
				RefInvoker.setFieldObject(pluginLoadedApk, loadedAPKClass, "mDataDirFile", new File(PluginLoader
						.getApplication().getApplicationInfo().dataDir));
				RefInvoker.setFieldObject(pluginLoadedApk, loadedAPKClass, "mDataDir", PluginLoader.getApplication()
						.getApplicationInfo().dataDir);
				// TODO 需要时再说
				// RefInvoker.setFieldObject(pluginLoadedApk, loadedAPKClass,
				// "mLibDir", );
			}
			// 再还原
	...
	public static Map<IBinder, Service> getAllServices() {
		Map<IBinder, Service> services = (Map<IBinder, Service>) RefInvoker.getFieldObject(currentActivityThread(),
				android_app_ActivityThread, android_app_ActivityThread_mServices);
		return services;
	}
