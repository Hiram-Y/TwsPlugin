...
public class ActivityThread {
	...
	public synchronized static Object currentActivityThread() {
		if (sCurrentActivityThread == null) {
			...
			if (sCurrentActivityThread == null) {
				...
				if (impl != null) {
					sCurrentActivityThread = RefInvoker.getField(impl, android_app_ContextImpl,
	...
	public static Object getResCompatibilityInfo() {
		...
		Object compatInfo = RefInvoker
				.getField(mBoundApplication, android_app_ActivityThread_AppBindData, "compatInfo");
		return compatInfo;
	}
	...
	public static Object getBoundApplicationData() {
		Object mBoundApplication = RefInvoker.getField(currentActivityThread(), android_app_ActivityThread,
				"mBoundApplication");
	...
	public static void wrapHandler() {
		...
		RefInvoker
				.setField(handler, Handler.class.getName(), android_os_Handler_mCallback, new PluginAppTrace(handler));
	}
	...
	public static void wrapInstrumentation() {
		...
		if (!(originalInstrumentation instanceof PluginInstrumentionWrapper)) {
			RefInvoker.setField(currentActivityThread(), clazz(), android_app_ActivityThread_mInstrumentation,
	...
	public static void installPackageInfo(Context hostContext, String pluginId, PluginDescriptor pluginDescriptor,
		...
		Object applicationLoaders = RefInvoker.invokeMethod("android.app.ApplicationLoaders", "getDefault",
		...
		try {
			...
			if (pluginLoadedApk != null) {
				...
				RefInvoker.setField(pluginLoadedApk, loadedAPKClass, "mDataDirFile", new File(PluginLoader
						.getApplication().getApplicationInfo().dataDir));
				RefInvoker.setField(pluginLoadedApk, loadedAPKClass, "mDataDir", PluginLoader.getApplication()
						.getApplicationInfo().dataDir);
				// TODO 需要时再说
				// RefInvoker.setField(pluginLoadedApk, loadedAPKClass,
				// "mLibDir", );
			}
			// 再还原
	...
	public static Map<IBinder, Service> getAllServices() {
		Map<IBinder, Service> services = (Map<IBinder, Service>) RefInvoker.getField(currentActivityThread(),
