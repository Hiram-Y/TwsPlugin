...
public class PluginViewInflater {
	...
	public final View createView(View parent, final String name, Context context, AttributeSet attrs,
			boolean inheritContext, boolean themeContext) {
		final Context originalContext = context;

		// We can emulate Lollipop's android:theme attribute propagating down
		// the view hierarchy
		...
		if (originalContext != context) {
			// If the original context does not equal our themed context, then
			// we need to manually
	...
	/**
	 * Allows us to emulate the {@code android:theme} attribute for devices
	 * before L.
	 */
