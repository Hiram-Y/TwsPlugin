...
public class PluginFactoryCompat {
	private static final String TAG = "rick_Print:FactoryCompat";
		...
		if (Build.VERSION.SDK_INT >= 11) {
			...
			if (Build.VERSION.SDK_INT < 21) {
				...
				if (f instanceof LayoutInflater.Factory2) {
					// The merged factory is now set to getFactory(), but not
					// getFactory2() (pre-v21).
					// We will now try and force set the merged factory to
					// mFactory2
	...
	/**
	 * For APIs >= 11 && < 21, there was a framework bug that prevented a
	 * LayoutInflater's Factory2 from being merged properly if set after a
	 * cloneInContext from a LayoutInflater that already had a Factory2
	 * registered. We work around that bug here. If we can't we log an error.
	 */
	static void forceSetFactory2(LayoutInflater inflater, LayoutInflater.Factory2 factory) {
		if (!sCheckedField) {
			try {
				...
				Log.e(TAG,
						"forceSetFactory2 Could not find field 'mFactory2' on class " + LayoutInflater.class.getName()
		...
		if (sLayoutInflaterFactory2Field != null) {
			try {
				...
				Log.e(TAG, "forceSetFactory2 could not set the Factory2 on LayoutInflater " + inflater
						+ "; inflation may have unexpected results.", e);
			}
		}
	}
