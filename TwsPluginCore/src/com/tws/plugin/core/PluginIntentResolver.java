...
public class PluginIntentResolver {
	...
	/* package */static String resolveServiceForClassLoader(Object msgObj) {
		...
		if (ProcessUtil.isPluginProcess()) {
			...
			TwsLog.d(TAG, "hackServiceName=" + info.name + " packageName=" + info.packageName + " processName=" + info.processName + " targetClassName="
					+ targetClassName + " applicationInfo.packageName=" + info.applicationInfo.packageName);
			...
			if (targetClassName != null) {
				...
				TwsLog.e(TAG, "hackServiceName 没有找到映射关系, 可能映射表出了异常 info.name=" + info.name + " dumpString=" + dumpString);
