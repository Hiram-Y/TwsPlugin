...
public class PluginAppTrace implements Handler.Callback {
	...
	private static final String TAG = "rick_Print:PluginAppTrace";
	...
	public boolean handleMessage(Message msg) {
		...
		TwsLog.d(TAG, ">>> handling: " + CodeConst.codeToString(msg.what));
		...
		try {
			...
			TwsLog.d(TAG, ">>> done: " + CodeConst.codeToString(msg.what));
			...
			if (activityThread != null) {
				...
				if (services != null) {
					...
					if (service != null) {
						...
						TwsLog.d(TAG, "beforeStopService unBindStubService:" + pluginServiceClassName);
	...
	private static void afterReceiver(Result result) {
		if (ProcessUtil.isPluginProcess()) {
			if (result != null && result.baseContext != null) {
				RefInvoker.setField(result.baseContext, "android.app.ContextImpl", "mReceiverRestrictedContext",
						null);
			}
		}
	}
