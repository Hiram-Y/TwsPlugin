...
public class PluginLauncher implements Serializable {
	...
	public LoadedPlugin startPlugin(PluginDescriptor pluginDescriptor) {
		...
		if (plugin == null) {
			...
			TwsLog.e(TAG, "正在初始化插: " + pluginDescriptor.getPackageName() + ": Resources, DexClassLoader, Context, Application");
			TwsLog.e(TAG, "插件信息 Ver:" + pluginDescriptor.getVersion() + " InstalledPath=" + pluginDescriptor.getInstalledPath());
			...
			TwsLog.e(TAG, "初始化插件:" + pluginDescriptor.getPackageName() + "完成");
		} else {
			// TwsLog.d("IS RUNNING", packageName);
		}
	...
	public void stopPlugin(String packageName, PluginDescriptor pluginDescriptor) {
		...
		//
		// 退出WebView,
		// LocalService、Activity、BroadcastReceiver、LocalBroadcastManager,
		// Service、AssetManager、ContentProvider、fragment
		//
